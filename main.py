...
listecountry = df['Country'].tolist()
countries=pd.unique(pd.Series(listecountry))
...
#liste_party = df['Party'].tolist()
#liste_epg = df['EPG'].tolist()
liste_result_votes = df['1'].tolist()
...
indice_votes = np.zeros((811, len(liste_result_votes)))

for i in range(len(liste_result_votes)):
    for j in range(len(votes)):     
        if liste_result_votes[i] == votes[j]:
            indice_votes[i][j] = 1
